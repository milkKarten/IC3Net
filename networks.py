...
class VectorQuantizer(nn.Module):
    ...
    def forward(self, latents):
        #Latents have shape [B X N_AGENTS X D]
        latents_shape = latents.shape
        flat_latents = latents.view(-1, self.D)  # [N_AGENTS x D]

        # Compute L2 distance between latents and embedding weights
        dist = torch.sum(flat_latents ** 2, dim=1, keepdim=True) + \
               torch.sum(self.embedding.weight ** 2, dim=1) - \
               2 * torch.matmul(flat_latents, self.embedding.weight.t())  # [N_AGENTS x K]

        # Get the encoding that has the min distance
        encoding_inds = torch.argmin(dist, dim=1).unsqueeze(1)  # [N_AGENTS, 1]

        # Convert to one-hot encodings
        device = latents.device
        encoding_one_hot = torch.zeros(encoding_inds.size(0), self.K, device=device)
        encoding_one_hot.scatter_(1, encoding_inds, 1)  # [N_AGENTS x K]

        # Quantize the latents
        quantized_latents = torch.matmul(encoding_one_hot, self.embedding.weight)  # [N_AGENTS, D]
        quantized_latents = quantized_latents.view(latents_shape)  # [N_AGENTS x D]
