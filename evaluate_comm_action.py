...
all_comms_to_loc1 = {}
all_comms_to_loc2 = {}
total_comm_actions = np.zeros((500,args.max_steps))
t_delta = 5
for i in range(500):
    ...
    for j in range(args.max_steps):
        if j < t_delta:
            total_comm_actions[i][j] += comm_action_episode[:t_delta].mean()
        elif j >= args.max_steps - t_delta:
            total_comm_actions[i][j] += comm_action_episode[-t_delta:].mean()
        else:
            total_comm_actions[i][j] += comm_action_episode[j-t_delta//2:j+t_delta//2].mean()
    # total_comm_actions[i] += comm_action_episode
print(total_comm_actions)
print(total_comm_actions.mean(axis=0), total_comm_actions.std(axis=0))
with open("total_comm_action_"+args.env_name+"_"+str(args.discrete_comm)+".txt", "a+") as f:
    f.write(str(total_comm_actions.mean(axis=0))[1:-1])
